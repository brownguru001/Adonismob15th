  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-12 sm:px-6 lg:px-8">
      ...
      <div className="mt-8 rounded-xl border border-ink/10 p-6">
        <p className="text-sm font-semibold uppercase tracking-wide text-ink/50">Timeline</p>
        <div className="mt-4 space-y-3">
          {customOrder.statusEvents.map((event) => (
            <div key={event.id} className="flex items-start gap-3 text-sm">
              <div className="mt-1.5 h-2 w-2 shrink-0 rounded-full bg-gold" />
              <div>
                <p className="font-medium text-ink">{event.status.replace("_", " ")}</p>
                {event.note && <p className="text-ink/50">{event.note}</p>}
                <p className="text-xs text-ink/40">{new Date(event.createdAt).toLocaleString()}</p>
              </div>
            </div>
          ))}
        </div>

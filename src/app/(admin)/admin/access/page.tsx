  ...
  return (
    <div>
      ...
      <div className="mt-6 overflow-x-auto rounded-xl border border-ink/10 bg-white">
            ...
            {invitations.map((inv) => {
              ...
              return (
                <tr key={inv.id}>
                  ...
                  <td className="px-4 py-3 text-ink/70">{inv.invitedBy.name}</td>
                  <td className="px-4 py-3">
                    <span className={`rounded-full px-2.5 py-1 text-xs ${STATUS_STYLE[status]}`}>{status}</span>
                  </td>
                  <td className="px-4 py-3 text-ink/50">{inv.expiresAt.toLocaleDateString()}</td>
        ...
        {invitations.length === 0 && <p className="p-8 text-center text-ink/40">No invitations yet.</p>}
      </div>
    </div>
  );
}

  ...
  return (
    <div>
      ...
      <div className="mt-6 grid gap-4 overflow-x-auto pb-4 lg:grid-cols-4">
        {grouped.map((group) => (
          <div key={group.stage} className="min-w-[220px] rounded-xl border border-ink/10 bg-white p-4">
            ...
            <div className="mt-3 space-y-2">
              {group.orders.map((p) => (
                <Link
                  ...
                  className="block rounded-lg border border-ink/10 p-3 text-xs hover:border-ink/30"
                >
                  <p className="font-medium">{p.order.orderNumber}</p>
                  <p className="text-ink/50">{p.order.user.name}</p>
                  <p className="text-ink/40">{p.supplier?.name ?? "Unassigned"}</p>
                </Link>
              ))}
            </div>
          </div>
